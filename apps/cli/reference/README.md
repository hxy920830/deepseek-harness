...
A packaged parent may set `DSH_PARENT_LIFETIME=stdio` and keep the child's stdin pipe open. The first input byte or EOF starts the same graceful shutdown after profile boot; any other value fails loud. This channel is for a process supervisor that owns stdin, not an interactive invocation.
