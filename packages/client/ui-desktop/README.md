...
Desktop-only window-close integration. The Host half registers `ui-desktop.closeBehavior` with `ask` as its default and accepts `ask`, `minimize`, or `exit`; the ordinary settings provider persists the selection in `$DSH_HOME/settings.yaml`. The client half appears only inside the official Tauri runtime, contributes the selector to General settings, and binds it through `ctx.settingsScope`, so the durable Host document remains the single preference owner and users can change the choice after saving it.
...
- **Tauri runtime only** — ordinary browser sessions receive no settings row, overlay, event listener, or native command invocation from this plugin.
