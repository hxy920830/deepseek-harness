...
每个 pi-ai 模型行都有两个级联选择框，用于明确选择能力来源：第一个只列出内置 pi-ai 提供方，第二个列出所选提供方已安装目录中的模型。编辑器不会根据自定义路由的模型 id 匹配来源。更换提供方会清除之前的模型；只有两个选择都完成后，才会写入 `capabilitiesFrom` 对象。选中的来源向适配器提供能力元数据，而自定义路由继续保留自己的 provider id、模型 id、API、端点、凭据及其他请求身份。
...
- **能力来源选择仅支持内置目录**：选择框使用已安装的 pi-ai 提供方/模型目录，不支持自定义路由，也不会自动匹配模型 id。
