...
### Reuse built-in model capabilities

Each pi-ai model row has two cascading selectors for an explicit capability source: the first lists only built-in pi-ai providers, and the second lists models from the selected provider's installed catalog. The editor never matches a source from the custom route's model id. Changing the provider clears the previous model; the saved `capabilitiesFrom` object is written only after both selections are complete. The selected source supplies capability metadata to the adapter, while the custom route keeps its own provider id, model id, API, endpoint, credentials, and other request identity.
...
- **Capability-source choices are built-in only** — the selectors use the installed pi-ai provider/model catalog and do not support custom routes or automatic model-id matching.
