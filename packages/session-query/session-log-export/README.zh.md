...
## 使用本包
弹窗报告准备中、下载完成或失败。原生保存的归档会显示绝对路径以及“打开文件夹”“打开文件”操作，并在数秒后自动关闭；浏览器下载显示短暂的成功状态后同样自动关闭。失败状态保持打开，直到用户关闭。关闭弹窗不会取消正在进行的下载；该操作随后完成时也不会重新打开弹窗。每个 Session 同时只允许一项下载，重复手势会共用该任务。
...
- **浏览器下载或桌面原生保存**——除非 Tauri 用户配置 `ui-desktop.sessionLogDir`，否则目标位置由浏览器选择；原生路径会把归档保存到所选文件夹且不会覆盖已有文件。
- **预检报告早期失败**——浏览器接受 GET 后发生的子会话或附件读取失败由浏览器下载管理器报告；原生保存路径会在弹窗中报告 GET 和写入失败。
- **归档字节以 JSON 跨桌面 IPC**——原生保存会先把 ZIP 编码为 JSON 数字数组再交给 Rust 写入，大型导出会增加开销。
